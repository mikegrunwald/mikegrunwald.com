	...
	renderBrowseTab() {
		...
		return window.h(
			...
			window.h(
				...
				window.h(
					'div',
					{ style: { fontSize: '12px', fontWeight: '500', color: '#798291', marginBottom: '6px' } },
					'File Type'
				),
				window.h(
					'div',
			...
			(this.state.searchQuery ||
				this.state.selectedFileType !== 'all' ||
...
const SmartMediaPreview = window.createClass({
	render() {
		...
		const isR2 =
			valueStr.includes('assets.mikegrunwald.com') || valueStr.includes('r2.cloudflarestorage.com');
		...
		return window.h(
			...
			window.h(
				...
				isInlineSVG ? '💾 Stored Inline (SVG Code)' : isR2 ? '☁️ Stored in R2' : '📁 Stored in Git'
			),
